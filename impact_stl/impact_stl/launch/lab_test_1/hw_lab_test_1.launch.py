...
def generate_launch_description():
    ...
    # Plotjuggler from the juggler_3.xml file (3 spacecrafts)
    ld.add_action(Node(
            package='plotjuggler',
            namespace='snap',
    ...
    ld.add_action(Node(
            ...
            namespace='crackle',
            output='screen',
            parameters=[{'topic_name': '/crackle/velocity'}]
    ))
    ld.add_action(Node(
            package='push_stl',
            ...
            namespace='crackle',
            output='screen',
            parameters=[{'topic_name': '/crackle/pose'}]
    ))
    # pop
    ld.add_action(Node(
            package='push_stl',
    ...
    ld.add_action(Node(
            ...
            namespace='pop',
            output='screen',
            parameters=[{'topic_name': '/pop/velocity'}]
    ))
    ld.add_action(Node(
            package='push_stl',
            executable='odom_to_vehicle_attitude',
            namespace='pop',
            output='screen',
            parameters=[{'topic_name': '/pop/pose'}]
    ))
