...
def csv_to_plan(robot_name,path=''):
    ...
    return rvars, hvars, ids


def zonotopes_to_csv(X0s,Xfs,ids,other_names,robot_name,path=''):
    assert (len(X0s) == len(Xfs))
    # write rvars (list of numpy array) and hvars
    # as well as the ids (indiciating pre-post-impact etc.)
    if robot_name[0] == '/':
        fn = f"zonotopes_{robot_name[1::]}.csv"
    else:
        fn = f"zonotopes_{robot_name}.csv"
...
def csv_to_zonotopes(robot_name,path=''):
